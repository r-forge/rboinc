...
options('repos')
```
Дополнительно добавляется репозиторий "https://cloud.r-project.org". Если пакет не удалось установить из этих репозиториев, то производится попытка установить пакет с BioConductor'а. Следующий пример использует пакет с BioConductor:
```{r  eval=FALSE}
library(RBOINC.cl)
