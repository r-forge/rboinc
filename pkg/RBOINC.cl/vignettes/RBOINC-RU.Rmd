...
### Досборка виртуальной машины
Предварительно собранные виртуальные машины могут быть найдены по этой ссылке: https://drive.google.com/drive/folders/1BNn8NJRPcVmahIqd0TBetxpKwEainS32?usp=sharing. В этих машинах установлен минимальный набор ПО и пакетов, необходимый для нормальной работы пакета RBOINC. Поддерживаются только архитекуры i686 и amd64. По этой же ссылке лежит файл виртуального диска с инструментом сборки новых виртуальных машин. Обратите внимание, что виртуальные машины содержат ПО, лицензия которого отличается от BSD3.

Сборка виртуалки для использования с RBOINC достаточно проста:

1. Скачать образ диска виртуальной машины с суфиксом "bs"

1. Создать в virtualbox виртуальную машину, подключить к ней скачаный диск и ещё 2 пустых диска (рекомендуемый размер - 9 Гб.)

1. Загрузиться. Войти как root (пароля нет).

1. В домашней дирректории root лежат несколько скриптов:

  1. "enter.sh" - этот скрипт нужен для изменения корневого каталога (входа) на файловую систему будующей виртуальной машины. Используйте его для установки и обновления ПО, и других действий при подготовке виртуальных машин.
  
  1. "prepare_update.sh" - этот скрипт обновляет ПО обоих виртуальных машин и инструмента сборки (за исключением ядра - его можно обновить только вручную). Дополнительно он устанавливает на виртуальные машины минимально необходимый набор пакетов языка R.
  
  1. "flash.sh" - этот скрипт подготавливает образ выбранной виртуальной машины и записывает его на диск.

1. Сначала выполните "prepare_update.sh". Следите за ошибками, которые возможны в процессе его выполнения.

1. По необходимости используйте "enter.sh" для переключения на необходимую виртуальную машину для выполнения дополнительных действий.

1. Если непобходима установка дополнительных пакетов R, пропишите их в файле "~/scripts/root/make_release.R".

1. Когда обновление и установка ПО завершена, используйте "flash.sh" для записи виртуальной машины на диск (диск должен быть пустым).

1. Проверьте корректность загрузки и завершения работы полученного диска RBOINC.
...
## [1] 9
```
### Параметр install_func {#install_func_desc}
...
fun = function()
{
  ...
  classes = matrix(c(1,      2,    3, 4, # servers
                     ...
                     1,    1.9,  2.5, 3), # internal class speed (1/mean work)
                   nrow = 3, byrow = TRUE)
...
Если после этого остались пакеты, которые не удалось устновить, их имена передаются функции [install_func](#install_func_desc)
...
## Функция cancel_jobs
Это функция для отмены пачки заданий. В качестве первого параметра ей должно быть передано открытое заранее функцией create_connection соединение. В качестве второго - параметра передаётся статус пачки работы, который возвращается функциями create_jobs или update_jobs_status. Обратите внимание, что второй параметр ссылка (как в C++), но также корректным является следующий вариант вызова этой функции:
```{r eval=FALSE}
