...
if (!$user_submit->submit_all){
    // User can't submit jobs for all task. Check privilegies for rboinc app:
    $app = BoincApp::lookup("name='rboinc'");
    $access = BoincUserSubmitApp::lookup("user_id=$user->id and app_id=$app->id");
    if(!$access){
